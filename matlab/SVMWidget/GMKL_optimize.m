function svm=GMKL_optimize(features,labels,parms)
...
global initx
if strcmpi(parms.TYPEsolver,'ASVM-bsb')
M = size(features, 2);
N = size(features,1)/2;
EPS_d = parms.EPSd;
EPS_v = parms.EPSv;

initx = zeros(2*M+2*N*M);

tmp = [];
for i=1:N
    tmp = [tmp,features(N+i,:),-features(N+i,:)];
end
parms.f=EPS_d*[zeros(2*M,1);ones(2*N*M,1)] + [labels;-labels; zeros(2*N*M,1)] + EPS_v*[ones(2*M,1);...
    zeros(2*N*M,1)] + [zeros(2*M,1);tmp'];

parms.lb = zeros(2*M+2*N*M,1);
parms.ub = [1*ones(2*M,1);1*ones(2*N*M,1)];
else if strcmpi(parms.TYPEsolver,'ASVM-osb')
        M = size(features, 2);
        N = size(features,1)/2;
        P = length(find(labels==1));
        parms.labels=labels;
        parms.f=[-ones(M,1);-parms.R*ones(P,1);zeros(N,1)];
        parms.Aeq=[labels;zeros(P+N,1)]';
        parms.beq=0;
        parms.lb=[zeros(M+P,1);-inf*ones(N,1)];
        parms.ub=[1*ones(M+P,1);inf*ones(N,1)];
    end
end
  [curpt,diagnostic]=PGDoptimize(features,labels,parms);
  
  if strcmpi(parms.TYPEsolver,'ASVM-bsb')
      parms.ub = [parms.Cv*ones(2*M,1);parms.Cd*ones(2*N*M,1)];
      curpt=solvesubprob(curpt.d, features, labels, parms);
  else if strcmpi(parms.TYPEsolver,'ASVM-osb')
          parms.ub=[parms.C*ones(M+P,1);inf*ones(N,1)];
          curpt=solvesubprob(curpt.d, features, labels, parms);
      end
  end
  svm=calSVMparms(curpt,[],diagnostic,labels,parms);
end
...
if strcmpi(parms.TYPEsolver,'LIB')
    curpt=LIBsolvesubprob(d,features,labels,parms);
elseif strcmpi(parms.TYPEsolver,'MAT')
    curpt=MATsolvesubprob(d,features,labels,parms);
elseif strcmpi(parms.TYPEsolver,'ASVM-bsb')
    curpt=ASVM_bsb_solversubprob(d, features, labels, parms);
elseif strcmpi(parms.TYPEsolver,'ASVM-osb')
    curpt=ASVM_osb_solversubprob(d, features, labels, parms);
else
    error('Unknown SVM solver requested.\n');
end
end

function curpt=ASVM_bsb_solversubprob(d,features,labels,parms)
global initx
  if (all(abs(d)<1e-5)),
    curpt.cost=Inf;
...
options=optimset('TolX',1e-10,'TolCon',1e-10,'Algorithm','interior-point-convex','Display',...
    'off','TolFun',1e-10,'MaxIter',2000,'MaxFunEvals',inf);

[alpha,minval,exitflag,output] = quadprog((Kopt+Kopt')/2, parms.f, [],[],[],[],parms.lb,parms.ub, [], options);
if (exitflag<1 && ~isempty(output.message)), warning(output.message); end;
...
function curpt=ASVM_osb_solversubprob(d,features,labels,parms)
global initx
  if (all(abs(d)<1e-5)),
    curpt.cost=Inf;
...
[alpha,minval,exitflag,output] = quadprog((Kopt+Kopt')/2, parms.f, [],[],parms.Aeq,parms.beq,parms.lb,parms.ub, [], options);
if (exitflag<1 && ~isempty(output.message)), warning(output.message); end;
curpt.cost=parms.fncR(parms,d)-minval;
curpt.x = alpha;
initx=curpt.x;
...
function curpt=MATsolvesubprob(d,features,labels,parms)
  ...
  if (all(abs(d)<1e-5)),
    ...
    Kopt=parms.fncK(features,features,parms,d);
      ...
      OPTIONS=optimset('Display','off','LargeScale','off','Algorithm','interior-point-convex','MaxIter',2000);
      ...
      if (isempty(bind)), warning('All alpha stuck at C. Inspect Kopt*Y*alpha to set bind (bind=svind?).');  end;

      curpt.d=d;
      curpt.svind=svind;
      curpt.alphay=alpha(svind).*labels(svind);
      curpt.b=mean(labels(bind).*(1-H(bind,:)*alpha));
      curpt.x=alpha.*labels;

    curpt.cost=parms.fncR(parms,d)-1/(parms.C)*minval;
  end;
end
...
function curpt=LIBsolvesubprob(d,features,labels,parms)
  if all(abs(d)<1e-5),
    ...
    libSVMstr='-h 0 -s 0 -t 4 -m 500';

    libSVMstr=sprintf('%s -c %f',libSVMstr,parms.C);

    
    model=svmtrain(labels,libSVMK,libSVMstr);
    cost= 0.5*model.sv_coef'*Kopt(model.SVs, model.SVs)*model.sv_coef - model.Label(1)*sum(model.sv_coef(1:model.nSV))...
        - model.Label(2)*sum(model.sv_coef(model.nSV(1)+1:end));
   
    if (isempty(model.SVs)), error('MKL:LIBerr','LIBSVM error: No support vectors found.'); end;
        
    curpt.cost=parms.fncR(parms,d)-1/(parms.C)*cost;
    curpt.svind=full(model.SVs);
    curpt.alphay=model.sv_coef;
    curpt.x = zeros(size(Kopt,1),1);
    curpt.x(model.SVs) = curpt.alphay';
    curpt.b=-model.rho;
    curpt.d=d;
  end;
end
...
if isempty(strfind(parms.TYPEsolver,'ASVM'))
    ...
    Kdashint=parms.fncKdashint(features,features,parms,curpt.d);
    for k=1:length(curpt.d),
        Kdashk=parms.fncKdash(features,features,parms,curpt.d,k,Kdashint);
        grad(k)=grad(k)-0.5*curpt.x'*Kdashk*curpt.x;
    end;
end
...
function printstats(iter,evals,step,curpt,diagnostic)
  if (isempty(diagnostic)),
      ...
      fprintf('[It. %02d] Gamma = [',iter);
      for i =1:length(curpt.d)
         fprintf('%6.2f',curpt.d(i));
      end

    fprintf('], Cost = %8.3f, PGM2 = %8.5f, ',curpt.cost,curpt.pgm2);
