...
#include "mex.h"
#include "mxUtil.h"
#include "../../include/asvm_nlopt_solver.h"
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
	if(nrhs == 0)
	{
		printf("Usage: mxNLOPTSolver ( data_cell_object, target_class, kernel_width, [parameter_file_name]\n");
		return;
	}
	...
	// Converting matlab data structure to asvmdata class instance
	asvmdata* dataobj = mxUtil_Data_mat2c(prhs[0]);
	dataobj->setParams("rbf", sigma, 1.0);

	dataobj->printToFile(".matlabdata.txt");
	dataobj->loadFromFile(".matlabdata.txt");
	asvm svmobj; ASVM_NLopt_Solver nlopt_solver;
	nlopt_solver.configure(paramfilename);
	nlopt_solver.learn(*dataobj, tclass, &svmobj);
