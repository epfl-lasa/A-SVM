...
int ASVM_SMO_Solver::learn(asvmdata input, unsigned int tclass, asvm* svmobj)
{

	unsigned int i,j;

	input.preprocess(tclass);

	if(bVerbose)
	{
		cout<<"Reading problem..."<<endl;
		cout<<"Dimension    : "<<input.dim<<endl;
		cout<<"Num Alpha    : "<<input.num_alpha<<endl;
		cout<<"Num Beta     : "<<input.num_beta <<endl;
	...
	init_warm_start(&input);


	dlabels = new int[M];
	for(i=0;i<M;i++)

		dlabels[i] = input.labels[i];
	...
	for(i=0;i<M+P+N;i++)
	{
		...
		for(j=0;j<M+P+N;j++)
			ker_matrix[i][j] = input.matkgh[i][j];
	...
	//			fprintf(file, "%lf  ", input.matkgh[i][j]);
	//		fprintf(file, "\n");
	//	}
	//	fclose(file);
	...
	#endif
	svmobj->setSVMFromOptimizedSol(x_smo, &input);
